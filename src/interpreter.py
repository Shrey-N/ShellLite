...
class Interpreter:
    ...
    def visit_OnRequest(self, node: OnRequest):
        # Register handler
        # path could be string or pattern
        if hasattr(node.path, 'value'): # simplistic check if it's string literal node?
             # But visit takes node. It should be evaluated
             path_val = self.visit(node.path)
        else:
             path_val = self.visit(node.path)
             
        self.http_routes[str(path_val)] = node.body
    ...
    def visit_Listen(self, node: Listen):
            ...
            def do_GET(self):
                ...
                if path in interpreter_instance.http_routes:
                    ...
                    try:
                         ...
                         self.send_response(200)
                         self.end_headers()
                         self.wfile.write(str(e.value).encode())
                    except Exception as e:
                         self.send_response(500)
                         self.end_headers()
                         self.wfile.write(str(e).encode())
                         
                    # Restore env? Threading makes this tricky if shared interpreter state.
                    # BaseHTTPRequestHandler is threaded? No, default HTTPServer is synchronous.
        ...
        print(f"ShellLite Server running on port {port}...")
        try:
